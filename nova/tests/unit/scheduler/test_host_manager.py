...
class HostStateTestCase(test.NoDBTestCase):
    ...
    def test_stat_consumption_from_instance(self, host_topo_mock,
        ...
        claim = host.consume_from_request(spec_obj)
        numa_fit_mock.assert_called_once_with(fake_host_numa_topology,
                                              claim.numa_topology,
                                              limits=None, pci_requests=None,
                                              pci_stats=None)
        numa_usage_mock.assert_called_once_with(host, spec_obj)
        sync_mock.assert_called_once_with(("fakehost", "fakenode"))
        ...
        self.assertEqual(((host, spec_obj),), numa_usage_mock.call_args)
        self.assertEqual(second_host_numa_topology, host.numa_topology)
        self.assertIsNotNone(host.updated)

    def test_stat_consumption_with_claim(self):
        memory_mb = 2048
        ram_ratio = 1.5
        vcpus = 2
        cpu_ratio = 1.5
        ...
        compute = objects.ComputeNode(
            ...
            local_gb_used=1, free_ram_mb=1024, vcpus=vcpus, vcpus_used=0,
            disk_available_least=None,
            updated_at=None, host_ip='127.0.0.1',
            hypervisor_type='htype',
            hypervisor_hostname='hostname', cpu_info='cpu_info',
        ...
        host._locked_consume_from_request(spec_obj)
        self.assertRaises(exception.ComputeResourcesUnavailable,
                          host._locked_consume_from_request,
                          spec_obj)

    def test_consumption_claim_numa_topology_fails(self):
        stats = {
            'num_instances': '5',
            'num_proj_12345': '3',
            'num_proj_23456': '1',
